// Matrix.  Copyright Steve Dunn (steve_dunn at ntlworld.com)
// Free for non-commercial use.  If used commercially, please seek permission first.
...
enum 	direction{	north,northeast,east,southeast,south,southwest,west,northwest, nowhere } ;

static struct _dir
{	direction dirStart ;	direction dirEnd ;int xStep ; int yStep ;} dirs[] =
{	{ north,					south,			0,	1			} ,
	{ south,					north,			0,	-1		} ,
...
typedef class CMarker* LPMARKER ;
...
class CExpandable{public:	virtual bool expand( const long n ) = 0 ;};
	...
	bool getDirectionSteps( long& x, long& y ,  direction dStart , direction dEnd ) 
	{
		...
		while( dirs[++i].dirStart != nowhere )
		{
			...
			if (	d.dirStart	== dStart &&	d.dirEnd		== dEnd )
				{	x = d.xStep ;		y = d.yStep ;	return true ;	}
		}
		return false ;
	}
...
public:
	...
	void debug(_bstr_t strName="???")
	{
		_bstr_t s ;
		s.Format("matrix (%s) ...   X=%d, Y=%d",strName, width(),height());
		AfxMessageBox(s);
	}
	ULONG width( ) const
	{ ULONG n = 0;
	...
	ULONG height( ) const	{	return ROW::size( ) ; }
	...
	T& cell( long x, long y )	
		{ if ( ! m_strMarker.length( )  )  return (*this)[y][x]  ; 
...
template <typename T>
CRow<T>& 
CMatrix<T>::operator[]( const ULONG n )
{
	if ( n >= ROW::size( )  )
		expand( n ) ;
...
template <typename T>
const CRow<T>& 
CMatrix<T>::operator[]( const ULONG n ) const 
{
	assert ( n < ROW::size( )  ) ;
	return *ROW::operator[]( n )  ;
}
...
template <typename T>
CMarker<T>&
CMatrix<T>::addMarker(	const _TCHAR* strName, 
												long x, 
												long y )
{
	XYSTATUS(x,y);
...
template <typename T>
const CCell<T>& 
CRow<T>::operator[]( const ULONG n ) const 
{
	assert(  n < size( )  ) ;
	return * ( COLUMN::operator[ ] ( n )  ) ;
}
...
template<typename T>
bool
CMatrix<T>::deleteRow( ULONG lRow )
{
	assert( lRow <height());
...
template<typename T>
bool
CMatrix<T>::deleteColumn( ULONG lColumn )
{
...
template<typename T>
bool
CMatrix<T>::insertRow( ULONG lPrevRow, ULONG lRows )
{
	if ( lPrevRow < height( )  )
	{
...
template<typename T>
bool
CMatrix<T>::insertColumn( ULONG lPrevColumn, ULONG lColumns )
{
	for ( ROW::iterator rowIt = ROW::begin(); rowIt!=ROW::end( ) ; rowIt++)
	{
